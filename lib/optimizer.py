...
class AdaSGDInf(Optimizer):
    """uses infinity norm instead
    """
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdaSGDInf, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(AdaSGDInf, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    if amsgrad:
                        ...
                        state['max_exp_avg_sq'] = torch.zeros(1) #torch.zeros_like(p.data)
                ...
                if amsgrad:
                    ...
                    torch.max(max_exp_avg_sq, torch.max(exp_avg_sq), out=max_exp_avg_sq)
                    # Use the max. for normalizing running avg. of gradient
                    denom = max_exp_avg_sq.sqrt().add_(group['eps'])
                else:
                    denom = exp_avg_sq.max().sqrt().add_(group['eps'])
